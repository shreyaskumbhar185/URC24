#include <PS3BT.h> // Include PS3 Controller Library
...
// --- PS3 Controller ---
BTD Btd(&Serial); // Bluetooth Dongle
PS3BT PS3(&Btd); // PS3 Controller Object
...
void loop() {
    if (PS3.PS3Connected) {
        PS3.PS3Update(); // Update the controller state
        
        if (PS3.getButtonPress(PS)) { // Check if the PS button is pressed to stop navigation
            stopMotors();
            Serial.println("Stopped by PS Button");
            return;
        }

        if (PS3.getButtonPress(UP)) { // Move forward when UP button is pressed
            moveForward();
            Serial.println("Moving Forward");
        } else if (PS3.getButtonPress(DOWN)) { // Move backward when DOWN button is pressed
            analogWrite(MOTOR_LEFT_BACKWARD, baseSpeed);
            analogWrite(MOTOR_RIGHT_BACKWARD, baseSpeed);
            Serial.println("Moving Backward");
        } else if (PS3.getButtonPress(LEFT)) { // Turn left when LEFT button is pressed
            turnLeft();
            Serial.println("Turning Left");
        } else if (PS3.getButtonPress(RIGHT)) { // Turn right when RIGHT button is pressed
            turnRight();
